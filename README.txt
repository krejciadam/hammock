...
Hammock is a java application, so that there should be no need to build it from source, unlike external
packages used (Clustal Omega, Hmmer3, HHsuite), that may need building on local machine. Refer Hammock 
manual for more info. 

If for some reason there is a need to build Hammock java code from source, it can be done this way: 
